...
class MLP(torch.nn.Module):
    def __init__(self, args):
        ...
        self.num_layers = args.num_mlp_layers
        self.use_batch_norm = args.use_batch_norm
        self.linear_list = torch.nn.ModuleList()
        self.batch_norm_list = torch.nn.ModuleList()

        self.linear_list.append(Linear(args.num_feats, args.dim_hidden))
        self.batch_norm_list.append(BatchNorm1d(args.dim_hidden))
        for _ in range(self.num_layers - 2):
            self.linear_list.append(Linear(args.dim_hidden, args.dim_hidden))
            self.batch_norm_list.append(BatchNorm1d(args.dim_hidden))
